    ...
    module SectionTypeEdit
      class LinkIdeasCell < Decidim::Plans::SectionEditCell
        ...
        def show
          ...
          unless snippets.any?(:ideas_picker)
            snippets.add(
              :ideas_picker,
              stylesheet_link_tag("decidim/ideas/idea_picker")
            )
            snippets.add(
              :ideas_picker,
              javascript_include_tag("decidim/ideas/idea_picker")
            )

            # This will display the snippets in the <head> part of the page.
            snippets.add(:head, snippets.for(:ideas_picker))
          end
        ...
        def data_picker(form, attribute, options = {}, prompt_params = {})
          ...
          unless options[:label] == false
            label_text = translated_attribute(section.body) + form.send(:required_for_attribute, field_attribute)
